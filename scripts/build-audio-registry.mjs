...
async function run() {
  ...
  if (!TOKEN) {
    ...
    console.warn('⚠️  Cannot fetch live audio from Vercel Blob.');
    
    if (!existsSync(REGISTRY_PATH)) {
      console.warn('⚠️  Creating a fallback audio-registry.json with existing known audio.\n');
      const fallback = [
        {
  ...
  const res = await fetch('https://blob.vercel-storage.com?prefix=companion/', {
    headers: { Authorization: `Bearer ${TOKEN}` }
  });

  if (!res.ok) {
    console.error(`Failed to fetch from Vercel Blob: ${res.status} ${res.statusText}`);
